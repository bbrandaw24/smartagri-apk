...
- UA 追加 `SmartAgriApp/1.0` 标识

## 编译
GitHub Actions 自动：push 到 main → `gradle assembleDebug` → APK 挂到 Release `apk-v1.0`。

## 安装
下载 `smartagri-v1.0-debug.apk` 到手机 → 点击安装（需允许「未知来源/安装未知应用」）。
debug 自签包，仅自用，不上架商店。
